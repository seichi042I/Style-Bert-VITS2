...
from losses import WavLMLoss, discriminator_loss, generator_loss, kl_loss
...
torch.backends.cuda.sdp_kernel("flash") # type: ignore
...
def run():
    ...
    if args.repo_id is not None:
        ...
        repo_id = hps.repo_id if hps.repo_id is not None else ""
        try:
            api.upload_file(
    ...
    if not args.skip_default_style:
        ...
        default_style.set_style_config(
            args.config, Path(os.path.join(config.out_dir, "config.json"))
        )
    ...
    if getattr(hps.train, "freeze_decoder", False):
        ...
        for param in net_g.enc_q.parameters():
            param.requires_grad = False
    ...
    if utils.is_resuming(model_dir):
        if net_dur_disc is not None:
            assert type(optim_dur_disc) == torch.optim.Optimizer
            try:
                ...
                if not optim_dur_disc.param_groups[0].get("initial_lr"):
                    optim_dur_disc.param_groups[0]["initial_lr"] = 0.0002
        ...
        if net_wd is not None:
            try:
                ...
                assert type(optim_wd) == torch.optim.Optimizer
                if not optim_wd.param_groups[0].get("initial_lr"):
                    optim_wd.param_groups[0]["initial_lr"] = wd_resume_lr
            except:
                assert type(optim_wd) == torch.optim.AdamW
                if not optim_wd.param_groups[0].get("initial_lr"):
                    optim_wd.param_groups[0]["initial_lr"] = 0.0002
        ...
        try:
            ...
            assert type(optim_g) == torch.optim.Optimizer and type(optim_d) == torch.optim.Optimizer
    ...
    assert (
        type(optim_g) == torch.optim.AdamW
        and type(optim_d) == torch.optim.AdamW
        # and type(optim_dur_disc) == torch.optim.AdamW
        and type(optim_wd) == torch.optim.AdamW
    ), f"g: {type(optim_g)}, d: {type(optim_d)}, dur: {type(optim_dur_disc)}, wd: {type(optim_wd)}"
    ...
    if net_dur_disc is not None:
        scheduler_dur_disc = torch.optim.lr_scheduler.LambdaLR(
            optim_dur_disc, lr_lambda=lr_lambda, last_epoch=scheduler_last_epoch # type: ignore
        )
    else:
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        ...
        scheduler_d.step()
        if net_dur_disc is not None:
            scheduler_dur_disc.step() # type: ignore
        if net_wd is not None:
            scheduler_wd.step() # type: ignore
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            # Discriminator
            with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
                loss_disc_all = torch.mean(torch.tensor([0.,0.]))
            if net_dur_disc is not None:
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
                ...
                if net_dur_disc is not None:
                    loss_dur_gen, losses_dur_gen = generator_loss(y_dur_hat_g) # type: ignore
                    if net_wd is not None:
                        loss_gen_all += loss_dur_gen + loss_lm + loss_lm_gen # type: ignore
                    else:
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0 and not hps.speedup:
                ...
                if net_dur_disc is not None:
                    scalar_dict.update({"loss/dur_disc/total": loss_dur_disc_all}) # type: ignore

                    scalar_dict.update(
                        {
                            "loss/dur_disc_g/{}".format(i): v
                            for i, v in enumerate(losses_dur_disc_g) # type: ignore
                        }
                    )
                    scalar_dict.update(
                        {
                            "loss/dur_disc_r/{}".format(i): v
                            for i, v in enumerate(losses_dur_disc_r) # type: ignore
                        }
                    )

                    scalar_dict.update({"loss/g/dur_gen": loss_dur_gen}) # type: ignore
                    scalar_dict.update(
                        {
                            "loss/g/dur_gen_{}".format(i): v
                            for i, v in enumerate(losses_dur_gen) # type: ignore
                        }
                    )
                ...
                #     scalar_dict.update(
                #         {
                #             "loss/wd/total": loss_slm, # type: ignore
                #             "grad_norm_wd": grad_norm_wd,# type: ignore
                #             "loss/g/lm": loss_lm,# type: ignore
                #             "loss/g/lm_gen": loss_lm_gen,# type: ignore
                #         }
                #     )
                image_dict = {
                ...
                utils.summarize(
                    ...
                    images=image_dict,
                    scalars=scalar_dict,# type: ignore
                )
            ...
            if (
                ...
                if not hps.speedup:
                    evaluate(hps, net_g, eval_loader, writer_eval)# type: ignore
                utils.checkpoints.save_checkpoint(
                    net_g,
                    optim_g,
                    hps.train.learning_rate,
